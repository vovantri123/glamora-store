...
-- ============================================
INSERT INTO product_reviews (product_id, variant_id, user_id, rating, comment, is_verified_purchase, is_deleted, created_at) VALUES
((SELECT product_id FROM product_variants WHERE sku = 'ATBC-DEN-M'), (SELECT id FROM product_variants WHERE sku = 'ATBC-DEN-M'), (SELECT id FROM users WHERE email = 'vovantri204@gmail.com'), 5, 'Áo rất đẹp, chất lượng tốt, form dáng vừa vặn. Shop giao hàng nhanh!', true, false, CURRENT_TIMESTAMP - INTERVAL '9 days'),
((SELECT product_id FROM product_variants WHERE sku = 'JEAN-DEN-30'), (SELECT id FROM product_variants WHERE sku = 'JEAN-DEN-30'), (SELECT id FROM users WHERE email = 'phamducan@gmail.com'), 4, 'Quần jean đẹp, nhưng hơi dài một chút. Nhìn chung ok.', true, false, CURRENT_TIMESTAMP - INTERVAL '12 hours'),
((SELECT product_id FROM product_variants WHERE sku = 'MAXI-HOA-M'), (SELECT id FROM product_variants WHERE sku = 'MAXI-HOA-M'), (SELECT id FROM users WHERE email = 'lethihoa@gmail.com'), 5, 'Đầm xinh lắm, chất vải mềm mại thoáng mát!', true, false, CURRENT_TIMESTAMP - INTERVAL '1 day');
